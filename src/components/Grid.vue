<template>
  <div :id="gridId" class="grid-stack">
    <griditem v-for="(item, i) in list" :item="item" :i="i" :key="i" :height="options.cellHeight"></griditem>
  </div>
</template>
  ...
  export default {
    ...
    mounted() {
      this.$nextTick( () => {
        ...
        this.grid = $("#" + this.gridId).gridstack(this.options)
